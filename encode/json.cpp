...
namespace daxia
{
	namespace encode
	{
		...
		void Json::putValueElement(const daxia::reflect::Reflect_base* reflectBase, const daxia::string& tag, boost::property_tree::ptree& root)
		{
			...
			boost::property_tree::ptree child;
			if (static_cast<const std::vector<unknow>&>(*array).empty())
			{
			...
			{
				int index = 0;
				const unknow* begin = reinterpret_cast<const char*>(&(*(static_cast<const std::vector<unknow>&>(*array).begin())));
				const unknow* end = begin + static_cast<const std::vector<unknow>&>(*array).size();
		...
		void Json::putObjectElement(const daxia::reflect::Reflect_base* reflectBase, const std::string& tag, boost::property_tree::ptree& root)
		{
			...
			if (layout.ElementSize())
			{
				...
				using daxia::reflect::Reflect;
				const Reflect<std::vector<unknow>>* array = reinterpret_cast<const Reflect<std::vector<unknow>>*>(reflectBase);
				layout.ElementCount() = static_cast<const std::vector<unknow>&>(*array).size() / layout.ElementSize();
			}

			boost::property_tree::ptree child;
			if (!static_cast<const std::vector<unknow>&>(*array).empty())
			{
				const char* baseaddr = reinterpret_cast<const char*>(&(*(static_cast<const std::vector<unknow>&>(*array).begin())));
				if (baseaddr != nullptr)
				{
